        ...
        <template #columns>
          <a-table-column title="月嫂照片" :width="100" align="center">
            <template #cell="{ record }">
              <div
                v-if="record.nursePhotoUrl && record.nursePhotoUrl.length"
                class="photo-cell"
              >
                <a-image
                  :src="record.nursePhotoUrl[0]"
                  width="56"
                  height="56"
                  fit="cover"
                />
          ...
          <a-table-column title="参考价格" :width="120" align="right">
            <template #cell="{ record }">
              <span v-if="record.price != null" class="price-text"
                >¥{{ record.price }}</span
              >
          ...
          <a-table-column title="状态" :width="120" align="center">
            <template #cell="{ record }">
              ...
              >
                <a-switch
                  :model-value="record.status === 'ONLINE'"
                  :loading="statusLoadingMap[record.nurseId]"
                  :disabled="statusLoadingMap[record.nurseId]"
                  type="round"
                  checked-color="rgb(var(--success-6))"
                  @change="
                    (val: boolean | (string | number)[]) =>
                      handleStatusChange(record, val as boolean)
                  "
                >
            ...
            <template #cell="{ record }">
              <a-space>
                ...
                <a-button
                  type="text"
                  size="small"
                  @click="handlePrintResume(record)"
                >
      ...
      <div class="resume-page">
        ...
        <div class="resume-header">
          <div class="resume-photo">
            <img
              v-if="printNurse?.nursePhotoUrl?.length"
              :src="printNurse.nursePhotoUrl[0]"
              alt="月嫂照片"
            />
          ...
          <div class="resume-info">
            ...
            <div class="resume-stars">
              <span
                ...
                :key="i"
                class="star"
                :class="{ filled: i <= (printNurse?.level || 0) }"
                >★</span
              >
        ...
        <div v-if="printNurse?.highlights?.length" class="resume-section">
          <div class="section-title"
            ><span class="title-icon">⭐</span> 个人亮点</div
          >
          <div class="section-content">
            <div class="highlights-list">
              <div
                v-for="(item, idx) in printNurse.highlights"
                :key="idx"
        ...
        <div v-if="printNurse?.nurseProfile" class="resume-section">
          <div class="section-title"
            ><span class="title-icon">📋</span> 个人简介</div
          >
          <div class="section-content">
        ...
        <div v-if="printNurse?.workExperience?.length" class="resume-section">
          <div class="section-title"
            ><span class="title-icon">💼</span> 工作经历</div
          >
          <div class="section-content">
            <div
              v-for="(exp, idx) in printNurse.workExperience"
              :key="idx"
        ...
        <!-- 专业技能 -->
        <div
          v-if="printNurse?.professionalSkills?.length"
          class="resume-section"
        >
          <div class="section-title"
            ><span class="title-icon">🎯</span> 专业技能</div
          >
          <div class="section-content">
            <div class="skills-grid">
              <div
                v-for="(skill, idx) in printNurse.professionalSkills"
                :key="idx"
                class="skill-card"
              >
        ...
        <!-- 证书图片 -->
        <div
          v-if="printNurse?.certificateImages?.length"
          class="resume-section"
        >
          <div class="section-title"
            ><span class="title-icon">📜</span> 资质证书</div
          >
          <div class="section-content">
            <div class="resume-gallery">
              <img
                v-for="(url, idx) in printNurse.certificateImages"
                :key="idx"
                :src="url"
        ...
        <div v-if="printNurse?.healthImages?.length" class="resume-section">
          <div class="section-title"
            ><span class="title-icon">🏥</span> 健康证明</div
          >
          <div class="section-content">
            <div class="resume-gallery">
              <img
                v-for="(url, idx) in printNurse.healthImages"
                :key="idx"
                :src="url"
                class="gallery-img gallery-img--cert"
              />
        ...
        <div v-if="printNurse?.packageName" class="resume-section">
          <div class="section-title"
            ><span class="title-icon">📦</span> 关联套餐</div
          >
          <div class="section-content">
        ...
        <div v-if="printNurse?.description?.length" class="resume-section">
          <div class="section-title"
            ><span class="title-icon">🖼️</span> 图文详情</div
          >
          <div class="section-content">
            <div class="resume-gallery">
              <img
                v-for="(url, idx) in printNurse.description"
                :key="idx"
                :src="url"
                class="gallery-img"
              />
